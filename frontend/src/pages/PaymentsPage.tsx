...
// Full replacement — fixes dark mode and wraps MerchantListings
// in a light-on-dark container so it renders visibly.
...
const TABS: {
...
}[] = [
  {
    ...
    label: 'Buy via P2P',
    icon: ShoppingBag,
    description: 'Buy crypto directly from verified merchants using local payment methods — lowest fees'
  },
  {
    id: 'fund',
    label: 'Fund Wallet',
    icon: ArrowDownToLine,
    description: 'Buy crypto instantly with your local currency via card or bank transfer'
  },
  {
    id: 'send',
    label: 'Send',
    ...
    <div className="flex h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 pt-20 lg:pt-6">
        <div className="max-w-5xl mx-auto">
          ...
          {/* ── Tab Bar ── */}
          <div className="flex gap-1 mb-6 bg-gray-800/60 rounded-xl p-1.5 border border-gray-700 w-fit">
            {TABS.map(tab => {
              const Icon = tab.icon
          ...
          {/* ── P2P Tab ──
              MerchantListings was built with light-mode classes (white cards, gray-900 text).
              We wrap it in a white rounded container so it renders correctly
              against the dark page background — no changes to MerchantListings needed.
          */}
          {activeTab === 'p2p' && (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm max-w-sm">
                  Buy crypto instantly with your local currency using card, bank transfer,
                  or mobile money. Powered by smart provider routing for best rates.
                </p>
              </div>
          ...
          {activeTab === 'send' && (
            <div className="flex flex-col items-center justify-center py-16 gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm max-w-sm">
                  Send any supported token to any wallet address. Smart routing
                  automatically selects the optimal chain. QR code scanning supported.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3 text-center text-sm w-full max-w-xs">
                {[
      ...
      <FundWalletModal
        open={showFund}
        onOpenChange={open => { setShowFund(open) }}
      />
      <SendForm
        open={showSend}
        onOpenChange={open => { setShowSend(open) }}
      />
    </div>
  )
}
