      ...
      const widget = moonPayInit({
        ...
        handlers: {
          async onInitiateDeposit(props: any) {
            // MoonPay calls this when it wants us to send crypto to their
            // deposit address. Log it for now — advanced: trigger wallet tx here.
            console.log('MoonPay deposit address:', props?.depositWalletAddress)
            return { depositId: `seamount-${Date.now()}` }
          },
          onCloseOverlay() {
            // user closed widget
          },
