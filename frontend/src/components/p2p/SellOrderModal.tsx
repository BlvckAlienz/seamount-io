...
// ═══════════════════════════════════════════════════════════════
// FILE 2: frontend/src/components/p2p/SellListings.tsx
// Shows merchant sell listings (merchants who want to buy tokens)
// ═══════════════════════════════════════════════════════════════
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center justify-between">
        <div className="flex gap-2">
          ...
          <select value={filterFiat} onChange={e => setFilterFiat(e.target.value)}
            className="text-sm px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
            {FIAT_OPTIONS.map(f => <option key={f} value={f}>{f || 'All Currencies'}</option>)}
          </select>
        </div>
        <button onClick={fetchListings} className="text-gray-400 hover:text-gray-600">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-3 text-xs text-gray-500 mt-0.5">
                  <span>{m?.total_orders} orders</span>
                  <span><Star className="h-3 w-3 inline text-yellow-400 mr-0.5" />{m?.completion_rate?.toFixed(1)}%</span>
                  <span><Clock className="h-3 w-3 inline mr-0.5" />{m?.avg_release_time_mins}min pay</span>
                </div>
              </div>
              <div className="text-right flex-shrink-0">
            ...
            <div className="flex items-center justify-between text-sm">
              <div className="space-y-0.5">
                <div className="text-xs text-gray-500">
                  Limit: <span className="text-gray-900 font-medium">
                    {listing.min_order_fiat?.toLocaleString()} – {listing.max_order_fiat?.toLocaleString()} {listing.fiat_currency}
                  </span>
                </div>
                <div className="text-xs text-gray-500">
                  Pays via: <span className="text-gray-900 font-medium">
