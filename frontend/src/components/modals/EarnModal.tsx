...
// âœ… PRODUCTION EARN MODAL - Folks Finance Integration
...
export const EarnModal: React.FC<EarnModalProps> = ({ open, onOpenChange }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto">
      ...
      {/* Modal - âœ… MATCHES YOUR FUND/WITHDRAW STYLE */}
      <div 
        className="relative bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 rounded-2xl p-6 w-full max-w-[600px] shadow-2xl animate-in slide-in-from-bottom-4 duration-300 my-8 mx-4"
        style={{ zIndex: 1000 }}
      >
        {/* Header */}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Icon className={`h-6 w-6 ${isSelected ? t.color : 'text-gray-500 dark:text-gray-400'}`} />
                    <span className="text-lg font-bold text-gray-900 dark:text-white">{t.name}</span>
                  </div>
                  {isSelected && <Check className={`h-5 w-5 ${t.color}`} />}
                </div>
                <div className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{t.apy}</div>
                <div className="text-xs text-gray-600 dark:text-gray-400 mb-2 font-medium">{t.risk}</div>
        ...
        {/* Stake Form */}
        <div className="bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 rounded-xl p-4 mb-4">
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-500 rounded-lg px-4 py-3 text-gray-900 dark:text-white h-12 focus:outline-none focus:ring-2 focus:ring-yellow-500"
            >
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-500 rounded-lg px-4 py-3 text-gray-900 dark:text-white text-lg h-12 focus:outline-none focus:ring-2 focus:ring-yellow-500"
            />
          </div>

          {/* Expected Returns */}
          {amount && parseFloat(amount) >= tier.minAmount && (
            <div className="bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 rounded-lg p-3 space-y-2">
              ...
              <div className="flex justify-between text-sm border-t-2 border-gray-200 dark:border-gray-600 pt-2">
        ...
        {/* Stake Button - âœ… SOLID COLOR */}
        <button
          onClick={handleStake}
          disabled={loading || !amount || parseFloat(amount) < tier.minAmount}
          className="w-full bg-yellow-600 hover:bg-yellow-700 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-semibold py-4 rounded-lg transition-all h-12 flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {userStakes
                ...
                .map((stake) => (
                  <div
                    key={stake.stake_id}
                    className="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3 flex justify-between items-center"
                  >
                    <div>
                      <div className="text-gray-900 dark:text-white font-medium">
                        {stake.principal} {stake.asset}
                      </div>
                      <div className="text-xs text-gray-600 dark:text-gray-400 capitalize">{stake.tier} Tier</div>
                    </div>
                    <div className="text-right">
                      <div className="text-green-600 font-medium">{stake.current_apy}</div>
