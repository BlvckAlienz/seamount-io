...
async def start_kyc_verification(
    ...
    # Priority market users need ID
    if country in ['NG', 'KE', 'GH'] and not current_user.get('id_number'):
        raise HTTPException(
            status_code=400,
            detail={
                "type": "missing_id",
                "message": f"ID verification required for {country} users",
...
async def submit_kyc_data(
    ...
    try:
        ...
        user_data = {
            ...
            'phone': data.get('phoneNumber'),
            'callback_url': f"{settings.API_BASE_URL}/api/v1/kyc/webhook"
        }
        ...
        raise HTTPException(status_code=500, detail=str(e))@router.post("/submit-kyc-data")
async def submit_kyc_data(
    data: dict,
    current_user: dict = Depends(get_current_user),
    supabase: Client = Depends(get_supabase_client),
