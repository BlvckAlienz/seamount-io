...
async def verify_token(token: str = Depends(oauth2_scheme), current_settings: Settings = Depends(get_settings)) -> Dict[str, Any]:
    try:
        ...
        if not key: raise JWTError("Public key for token not found in JWKS.")
...
async def get_current_user(payload: Dict[str, Any] = Depends(verify_token), supabase: Client = Depends(get_supabase_client)) -> Dict[str, Any]:
    ...
    if not user_id: raise HTTPException(status_code=401, detail="Invalid token payload: missing user identifier.")
    try:
...
# --- 5. FASTAPI APP ---
app = FastAPI(title="Seamount.io API", version="2.1.0", lifespan=lifespan)
...
def health_check(): return {"status": "healthy", "version": "2.1.0"}
...
async def initialize_session(request: Request, user_agent: Optional[str] = Header(None, alias="User-Agent"), supabase: Client = Depends(get_supabase_client), current_settings: Settings = Depends(get_settings)):
    try:
        ...
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=5)) as http_session:
                async with http_session.get(f"https://ipinfo.io/{ip_address}?token={current_settings.IPINFO_TOKEN.get_secret_value()}") as response:
                    if response.status == 200: ip_data = await response.json(); session_data.update({"isp": ip_data.get("org"), "country": ip_data.get("country"), "city": ip_data.get("city"), "is_vpn": ip_data.get("privacy", {}).get("vpn", False)})
        except Exception as e: logger.warning(f"IPinfo enrichment failed: {e}")
        
        insert_res = supabase.from_("user_sessions").insert(session_data).execute()
        if not insert_res.data: raise Exception("Failed to create session record in database.")
        ...
        error_id = str(uuid4())[:8]; logger.error(f"Session initialization failed [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Could not initialize session. Error ID: {error_id}")
...
async def investor_contact(payload: InvestorContactPayload, supabase: Client = Depends(get_supabase_client), notifier: NotificationService = Depends(get_notification_service)):
    try:
        ...
        subject = f"New Investor Contact: {payload.name}"; body = f"<html><body><p>Name: {payload.name}</p><p>Email: {payload.email}</p></body></html>"
        ...
        error_id = str(uuid4())[:8]; logger.error(f"Investor contact submission failed [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Could not process your request. Error ID: {error_id}")
...
def get_user_profile(current_user: Dict[str, Any] = Depends(get_current_user)):
    try: return UserProfile(**current_user)
    except Exception as e:
        error_id = str(uuid4())[:8]; logger.error(f"Failed to process user profile for {current_user.get('id')} [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Error processing profile data. Error ID: {error_id}")
...
async def update_kyc(submission: KYCSubmission, current_user: Dict[str, Any] = Depends(get_current_user), notifier: NotificationService = Depends(get_notification_service), supabase: Client = Depends(get_supabase_client)):
    try:
        user_id = current_user["id"]; kyc_data = {"user_id": user_id, "document_type": submission.document_type, "document_data": submission.document_data, "status": "pending"}
        ...
        asyncio.create_task(notifier.send_kyc_update(current_user["email"], "in_progress", "Your documents have been received and are now under review."))
        return {"message": "KYC documents submitted for review."}
    except Exception as e:
        error_id = str(uuid4())[:8]; logger.error(f"KYC submission failed for user {current_user.get('id')} [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Failed to submit KYC documents. Error ID: {error_id}")
...
async def send_payment(req: PaymentRequest, current_user: Dict[str, Any] = Depends(get_current_user), supabase: Client = Depends(get_supabase_client), notifier: NotificationService = Depends(get_notification_service)):
    try:
        if req.recipient_email == current_user.get("email"): raise HTTPException(status_code=400, detail="Cannot send payments to yourself.")
        ...
        asyncio.create_task(notifier.send_transfer_notifications(current_user["email"], req.recipient_email, req.amount, fee=0.01))
        return PaymentResponse(**tx_data, timestamp=datetime.utcnow())
    except HTTPException as he: raise he
    except Exception as e:
        error_id = str(uuid4())[:8]; logger.error(f"Payment failed for user {current_user.get('id')} to {req.recipient_email} [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Payment processing failed. Error ID: {error_id}")
...
def get_portfolio(current_user: Dict[str, Any] = Depends(get_current_user), supabase: Client = Depends(get_supabase_client)):
    try:
        ...
        error_id = str(uuid4())[:8]; logger.error(f"Portfolio fetch failed for user {current_user.get('id')} [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Failed to fetch portfolio. Error ID: {error_id}")
...
async def setup_mfa(current_user: Dict[str, Any] = Depends(get_current_user), supabase: Client = Depends(get_supabase_client)):
    try:
        ...
        error_id = str(uuid4())[:8]; logger.error(f"MFA setup failed for user {current_user.get('id')} [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Failed to set up MFA. Error ID: {error_id}")
...
async def verify_mfa(req: MFAVerifyRequest, current_user: Dict[str, Any] = Depends(get_current_user), supabase: Client = Depends(get_supabase_client)):
    try:
        ...
        if not mfa_res.data or not pyotp.TOTP(mfa_res.data["secret"]).verify(req.token): raise HTTPException(status_code=401, detail="Invalid MFA token")
        ...
        error_id = str(uuid4())[:8]; logger.error(f"MFA verification failed for user {current_user.get('id')} [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"MFA verification failed. Error ID: {error_id}")
...
def get_all_users(supabase: Client = Depends(get_supabase_client)):
    try:
        ...
        error_id = str(uuid4())[:8]; logger.error(f"Admin failed to fetch users [{error_id}]: {e}\n{traceback.format_exc()}"); raise HTTPException(status_code=500, detail=f"Could not retrieve users. Error ID: {error_id}")